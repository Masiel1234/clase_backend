...
class DashBoardInvDiaBatGenericaCalculoController extends Controller
{
    // KPIs principales del dashboard
    public function estadisticasGenerales()
    {
        $stats = InvDiaBatGenerica::query()->select(
    ...
    {
        $data = InvDiaBatGenerica::select('id_marca_fk', DB::raw('SUM(cantidad) as stock_total'))
            ->groupBy('id_marca_fk')
    ...
    {
        $data = InvDiaBatGenerica::select('calidad', DB::raw('SUM(cantidad) as stock_total'))
            ->groupBy('calidad')
            ->orderByDesc('stock_total')
            ->get();
    ...
    {
        return response()->json(
            InvDiaBatGenerica::where('cantidad', '<=', 5)
                ->orderBy('cantidad')
                ->limit(10)
                ->get()
        );
    ...
    {
        return response()->json(
            InvDiaBatGenerica::selectRaw('id, id_marca_fk, version, cantidad, costo, v_mayor, 
