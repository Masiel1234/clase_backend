...
class DashBoardInvDiaBatOriginalCalculoController extends Controller
{
    // KPIs principales del dashboard
    public function estadisticasGenerales()
    {
        $stats = InvDiaBatOriginal::select(
    ...
    {
        $data = InvDiaBatOriginal::select('id_marca_fk', DB::raw('SUM(cantidad) as stock_total'))
            ->groupBy('id_marca_fk')
    ...
    {
        $data = InvDiaBatOriginal::select('calidad', DB::raw('SUM(cantidad) as stock_total'))
            ->groupBy('calidad')
            ->orderByDesc('stock_total')
            ->get();
    ...
    {
        return response()->json(
            InvDiaBatOriginal::where('cantidad', '<=', 5)
                ->orderBy('cantidad')
                ->limit(10)
                ->get()
        );
    ...
    {
        return response()->json(
            InvDiaBatOriginal::selectRaw('id, id_marca_fk, version, cantidad, costo, v_mayor, 
